...
def sp_components(savefolder, savename):
    ...
    (
        sp_500_data.filter(["Symbol", "Security", "GICS_Sector"]).to_excel(
            os.path.join(savefolder, savename), index=False, freeze_panes=(1, 1)
        )
    )
...
def save_data_from_web(nr_of_years, savefolder, savename_comp, savename_hist_data):
    ...
    for share in tickers:
        try:
            data[share] = web.DataReader(share, data_source="yahoo", start=start_date)[
                "Adj Close"
            ]
            ...
            time.sleep(3)
            try:
                data[share] = web.DataReader(
                    share, data_source="yahoo", start=start_date
                )["Adj Close"]
            except:
    ...
    (
        ...
        .to_excel(
            os.path.join(savefolder, savename_hist_data),
            index=True,
            freeze_panes=(1, 1),
        )
    )
