...
def data_analysis(savefolder,
    ...
    correlation_table, random_portfolio_performance = random_portfolio_tables(sectors,
                                                                          sp_500,
                                                                          log_returns,
                                                                          time_series,
                                                                          savefolder,
    ...
    random_portf_charts(correlation_table, random_portfolio_performance, savefolder, corr_table, perf_compare_table)
...
def random_portfolio_tables(sectors, sp_500, log_returns, time_series, savefolder, savename_portf_analysis):
    ...
    random_portfolio_performance = 100 * (random_portfolio_prices / random_portfolio_prices.iloc[0])       
    
    random_portfolio_performance["Equal_weighted_rand_portf"] = random_portfolio_performance.iloc[:, 1:].mean(axis = 1)
    
    random_portfolio_performance.index = [conv_date(x.year, x.month, x. day) for x in random_portfolio_performance.index]
    ...
    with pd.ExcelWriter(os.path.join(savefolder, savename_portf_analysis), engine='xlsxwriter') as writer2:
        ...
        random_portfolio_performance.to_excel(writer2, sheet_name = "performance_comparison", float_format = '%.4f')
...
def random_portf_charts(correlation_table,
    ...
    plt.plot(random_portfolio_performance.index, market_portfolio, color='green', linewidth = 3)
    plt.plot(random_portfolio_performance.index, rand_shares, linestyle = "--", linewidth = 2)
    plt.plot(random_portfolio_performance.index, equal_weighted_rand_portf, color='blue', linewidth = 3)
