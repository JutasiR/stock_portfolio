...
def main_program():
    ...
    ans1 = input("Would you like to see the parameter names, their default values and descriptions? (Y/N)? ")
    ...
    if ans2.lower() == 'y':
        ...
        stock_nr = int(input("Number of stocks chosen during the portfolio generation (7): "))
        nr_of_modeled_portf = int(input("Number of randomly chosen set of stocks /portfolio/ (1000): "))
        nr_of_weights_per_portf = int(input("Number of randomly generated weights in all set of stocks (800): "))
    ...
    pg.portfolio_creator_usd_stocks(stock_nr,
                                    nr_of_modeled_portf,
    ...
    print("Overall Run time: {0} min {1} sec\n".format(int(np.round(end_total // 60, 0)),
                                                       int(np.round(end_total % 60, 0))))
